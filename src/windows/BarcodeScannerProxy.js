...
function videoPreviewRotationLookup(displayOrientation, isMirrored, rotateCameraPreview) {
    ...
    switch (displayOrientation) {
        case Windows.Graphics.Display.DisplayOrientations.landscape:
            degreesToRotate = (0 + (rotateCameraPreview || 0)) % 360;
            break;
        case Windows.Graphics.Display.DisplayOrientations.portrait:
            if (isMirrored) {
                degreesToRotate = (270 + (rotateCameraPreview || 0)) % 360;
            } else {
                degreesToRotate = (90 + (rotateCameraPreview || 0)) % 360;
            }
            break;
        case Windows.Graphics.Display.DisplayOrientations.landscapeFlipped:
            degreesToRotate = (180 + (rotateCameraPreview || 0)) % 360;
            break;
...
module.exports = {
    ...
    scan: function (success, fail, args) {
        ...
        function updatePreviewForRotation(evt) {
            ...
            var rotDegree = videoPreviewRotationLookup(
                currentOrientation,
                previewMirroring,
                BarcodeReader.scanCallArgs.args.length ? BarcodeReader.scanCallArgs.args[0].rotateCameraPreview : undefined,
            );
